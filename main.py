...
# --- 2️⃣ محرك التحليل السريع (Fast Analysis) ---
async def get_ultimate_analysis(symbol: str, tf_key: str):
    try:
        intervals = {"5m": Interval.INTERVAL_5_MINUTES, "15m": Interval.INTERVAL_15_MINUTES, "1h": Interval.INTERVAL_1_HOUR, "4h": Interval.INTERVAL_4_HOURS, "1d": Interval.INTERVAL_1_DAY, "1W": Interval.INTERVAL_1_WEEK, "1M": Interval.INTERVAL_1_MONTH}
...
🔥 **هدف 2:** {round(op_p*1.6, 2)}
🚀 **هدف 3:** {round(op_p*2.0, 2)}
🛑 **الوقف:** {round(op_p*0.7, 2)}
---
📢 @Option_Strike01"""
    except Exception as e: return f"❌ السهم {symbol} غير مدعوم حالياً أو هناك ضغط."

# --- 3️⃣ المعالج الرئيسي (بدون تأخير) ---
...
async def telegram_webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        if c_data.startswith("tf_"):
            ...
            # إرسال رسالة "جاري القنص" عشان ما تحس بالبطء
            asyncio.create_task(send_async_msg(chat_id, "⏳ جاري قنص الأهداف والسيولة..."))
...
async def send_menu(chat_id):
    menu = {"inline_keyboard": [[{"text": "🔍 رادار التحليل والسترايك", "callback_data": "menu_analyze"}], 
    ...
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    async with httpx.AsyncClient() as client:
        await client.post(url, json={"chat_id": chat_id, "text": "🛡 **غرفة عمليات Option Strike v7.8**", "reply_markup": menu})
...
async def send_frames(chat_id, txt):
    ...
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    async with httpx.AsyncClient() as client:
        await client.post(url, json={"chat_id": chat_id, "text": f"🎯 اختر فريم {txt}:", "reply_markup": grid})

@app.get("/")
async def home(): return {"status": "Fast & Responsive"}
