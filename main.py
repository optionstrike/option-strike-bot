...
# --- 1️⃣ جلب بيانات العقد (تحسين السرعة) ---
async def get_option_data(symbol, side):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            data = resp.json()
            if data.get('s') == 'ok' and len(data.get('strike', [])) > 0:
                return {
                    "strike": data['strike'][0], 
                    "price": data['mid'][0], 
                    "expiry": data['expiration'][0]
                }
    except: return None
    return None

# --- 2️⃣ محرك التحليل (تصحيح أسماء الفريمات وحل التعليق) ---
async def get_ultimate_analysis(symbol: str, tf_key: str):
    try:
        # تصحيح المسميات لتطابق المكتبة تماماً
        intervals = {
            "5m": Interval.INTERVAL_5_MINUTES,
            "15m": Interval.INTERVAL_15_MINUTES,
            "1h": Interval.INTERVAL_1_HOUR,
            "4h": Interval.INTERVAL_4_HOURS, # تأكد من وجود الـ S في النهاية
        ...
        # محاولة التحليل عبر أكثر من مصدر لضمان عدم التعليق
        ...
        # حساب الأهداف الفنية للسهم (Pivot Points)
        ...
        # جلب بيانات العقد بشكل متوازي لسرعة الرد
...
⚡️ **القاما:** {st1} | 📊 **الاحتمالية:** {"84%" if rsi > 60 else "62%"}

🎯 **أهداف السهم:** {st1} ➔ {st2} ➔ {st3}

📦 **السترايك المقترح:** {opt['strike'] if opt else 'ATM'}
💵 **دخول العقد:** {op_p}
...
📢 @Option_Strike01"""
        ...
        print(f"Error: {e}")
        return f"❌ تعذر تحليل {symbol} على فريم {tf_key}. جرب فريم آخر أو تأكد من الرمز."

# --- 3️⃣ نظام المعالجة والردود ---
...
async def telegram_webhook(request: Request):
    ...
    if "callback_query" in data:
        ...
        if c_data.startswith("tf_"):
            ...
            asyncio.create_task(send_async_msg(chat_id, f"⏳ جاري قنص بيانات {sym}..."))
        ...
        if txt in ["/START", "START"]:
            ...
            await send_frames(chat_id, txt)
            
    return {"ok": True}

async def send_async_msg(chat_id, text):
...
async def send_menu(chat_id):
    ...
    await send_async_msg_with_markup(chat_id, "🛡 **لوحة تحكم Option Strike v8.0**", menu)

async def send_frames(chat_id, txt):
    grid = {"inline_keyboard": [
        [{"text": "5m", "callback_data": f"tf_5m_{txt}"}, {"text": "1h", "callback_data": f"tf_1h_{txt}"}, {"text": "4h", "callback_data": f"tf_4h_{txt}"}],
        [{"text": "1d", "callback_data": f"tf_1d_{txt}"}, {"text": "أسبوعي", "callback_data": f"tf_1W_{txt}"}, {"text": "شهري", "callback_data": f"tf_1M_{txt}"}]
    ]}
    await send_async_msg_with_markup(chat_id, f"🎯 اختر فريم {txt}:", grid)

async def send_async_msg_with_markup(chat_id, text, markup):
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    async with httpx.AsyncClient() as client:
        await client.post(url, json={"chat_id": chat_id, "text": text, "reply_markup": markup})

@app.get("/")
async def home(): return {"status": "V8.0 Stable Deployment"}
